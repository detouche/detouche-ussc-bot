...
from database.connection_db import create_profile, get_competencies_list, add_competence_in_profile, get_profile_id, \
    remove_competence_from_profile


@rt.message(Text('Создать профиль'))
async def add_profile(message: types.Message, state: FSMContext):
...
async def add_competencies_in_profile(message: types.Message, state: FSMContext):
    title_data = await state.get_data()
    profile_id = get_profile_id(title_data['title'])
