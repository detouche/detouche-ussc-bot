...
def add_competence(competence_name, competence_description):
...
def add_competence_in_profile(competence_id, profile_id):
    try:
        ...
        status_add = cursor.execute(f"SELECT competence_id, profile_id FROM competence_profile "
                                    f"WHERE competence_id = ? AND profile_id = ?",
                                    (competence_id, profile_id)).fetchone()
...
def remove_competence_from_profile(competence_id, profile_id):
    cursor.execute(
        f"DELETE FROM competence_profile WHERE competence_id = ? AND profile_id = ?",
        (competence_id, profile_id))
    conn.commit()


def check_profile(profile_id):
    status = cursor.execute("SELECT profile_id FROM profiles WHERE profile_id = ?", (profile_id,)).fetchone()
    if status is None:
        return False
    else:
        return True
...
def change_competence_title(competence_id, new_competence_name):
    ...
    if status_title is None and status_id is not None:
        cursor.execute(
            ...
            f"SET competence_name = ? WHERE competence_id = ?", (new_competence_name.casefold(), competence_id))
        conn.commit()
        return True
    else:
        return False
...
def change_competence_description(competence_id, new_competence_description):
    ...
    if status_id is not None:
        cursor.execute(f"UPDATE competencies "
                       f"SET competence_description = ? "
                       f"WHERE competence_id = ?", (new_competence_description, competence_id))
        conn.commit()
        return True
    else:
        return False
...
def change_profile_title(profile_id, new_profile_name):
    ...
    if status_title is None and status_id is not None:
        cursor.execute(f"UPDATE profiles SET profile_name = ? "
                       f"WHERE profile_id = ?", (new_profile_name.casefold(), profile_id))
        conn.commit()
        return True
    else:
        return False
...
def competence_in_profile(profile_id, competence_id):
    try:
        status = cursor.execute(f"SELECT competence_id, profile_id FROM competence_profile "
                                f"WHERE profile_id = ? AND competence_id = ?", (profile_id, competence_id)).fetchone()
    except sqlite3.OperationalError:
        return False
    if status is None:
        return False
...
def delete_competence_from_profile(competence_id, profile_id):
    status = cursor.execute(
        ...
        f"WHERE competence_id = ? AND profile_id = ?", (competence_id, profile_id)).fetchone()
    if status is None:
        return False
    else:
        cursor.execute(
            f"DELETE FROM competence_profile WHERE competence_id= ? AND profile_id = ?", (competence_id, profile_id))
        conn.commit()
        return True
...
def get_assessment_comp_session(competence_name, user_id):
    assessment = cursor.execute(f'SELECT grade FROM user_session WHERE user_id = ? '
                                f'AND competence_name = ?', (user_id, competence_name)).fetchone()
    return assessment[0]


def get_id_comp_session(competence_name, user_id):
    competence_id = cursor.execute(f'SELECT user_session_id FROM user_session WHERE user_id = ? '
                                   f'AND competence_name = ?', (user_id, competence_name)).fetchone()
...
def transform_grade_current_comp(competence_id, new_grade):
    cursor.execute(f"UPDATE user_session SET grade = ? WHERE user_session_id = ?", (new_grade, competence_id))
    conn.commit()


def get_session_user_id(connection_code):
    user_id = cursor.execute(f"SELECT user_id FROM user_session WHERE connection_code = ? "
                             f"GROUP BY user_id", (connection_code,)).fetchall()
    return user_id


def get_competence_names(connection_code):
    competence_names = cursor.execute(
        f"SELECT competence_name FROM user_session WHERE connection_code = ? "
        f"GROUP BY competence_name", (connection_code,)).fetchall()
...
def get_competence_grades(competence_name, connection_code):
    return cursor.execute(f"SELECT grade FROM user_session WHERE competence_name = "
                          f"? AND connection_code = ?", (competence_name, connection_code)).fetchall()


def get_user_grades(user_id, connection_code):
    return cursor.execute(f"SELECT competence_name, grade FROM user_session "
                          f"WHERE user_id = ? AND connection_code = ?", (user_id, connection_code)).fetchall()
